...
5. **Sync `beta`** — `npm run release:sync-beta`. Merges `main` back into `beta`. A rebase would phantom-conflict because the squash commit on `main` already contains the combined effect of `beta`'s individual commits.
