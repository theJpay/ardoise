...
function StorageErrorBanner({ onRetry }: StorageErrorBannerProps) {
    return (
        <div className="bg-danger-surface border-danger-border text-ui-sm flex h-9 shrink-0 items-center justify-between border-b px-5 font-mono">
            ...
            <button
                className="text-danger border-danger-border text-ui-sm hover:bg-danger-surface-hover rounded-sm border bg-transparent px-2 py-0.5 font-mono transition-colors"
                onClick={onRetry}
            >
                Retry
            </button>
