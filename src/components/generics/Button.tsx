...
function Button({ label, variant = "primary", icon: Icon, onClick }: ButtonProps) {
    return (
        <button
            className={`text-ui-base duration-fast flex h-8 shrink-0 items-center justify-center gap-1.5 rounded px-4 font-medium transition-colors ${VARIANT_CLASSES[variant]}`}
            onClick={onClick}
        >
